...
const Billing = () => {
    ...
    const [billingType, setBillingType] = useState('direct'); // 'direct', 'appointment', 'new_customer'
    ...
    const [formData, setFormData] = useState({
        ...
        // For appointment-based billing
        appointment: '',

        // For customer-based billing (existing)
        customer_id: '',

        // For new customer billing
        phone: '',
        first_name: '',
        last_name: '',
        email: '',
    ...
    const fetchInvoices = async () => {
        setLoading(true);
        try {
            const response = await axiosInstance.get('/invoices/get-all-invoices/');
    ...
    const handleCreateInvoice = async (e) => {
        ...
        try {
            const response = await axiosInstance.post('/invoice/create-invoice/', payload);
        ...
        <DashboardLayout>
            <div className="min-h-screen bg-white">
                <div className="px-4">
                    ...
                    {/* Create Invoice Button - Aligned with header */}
                    <div className="flex justify-end mb-6">
                        <button
                            ...
                            className="bg-black text-white font-semibold py-2 px-6 rounded-lg transition-all duration-300 shadow-md hover:shadow-lg flex items-center gap-2 text-sm"
                        >
                    ...
                    {/* Create Invoice Form Modal - Matching Branch Form Design */}
                    {showCreateForm && (
                                                        ...
                                                        >
                                                            ...
                                                            {appointments.map(app => (
                                                                <option key={app.id} value={app.id}>
                                                                    #{app.id} - Customer: {app.customer} - {app.date}
                                                                </option>
                                                            ))}
                                                        </select>
                                                    </div>
                                            ...
                                            <div>
                                                ...
                                                <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                                                    ...
                                                    <button
                                                        ...
                                                        className="mt-3 w-full h-10 bg-black text-white text-sm font-semibold rounded-lg hover:bg-gray-800 transition-colors"
                                                    >
                                                        Add to Cart
                                                    </button>
                                                </div>
                    ...
                    {/* Invoice Details Modal - Matching Branch Form Design */}
                            ...
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {invoices.map((invoice, index) => {
                                        const branch = branches.find(b => b.id === invoice.branch);
                                        ...
                                        return (
                                            <tr key={invoice.id} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    <span className="text-sm text-gray-700">{branch?.name || `ID: ${invoice.branch}`}</span>
                                                </td>
                                                <td className="px-6 py-4">
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    ...
                                                    {parseFloat(invoice.discount) > 0 && (
                                                        <div className="text-xs text-green-600">-{invoice.discount}%</div>
                                                    )}
                                                </td>
                                                <td className="px-6 py-4 text-right">
